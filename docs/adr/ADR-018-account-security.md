...
| **D10** | **A login declares its own actor to the audit row.** `AuditLoggingMiddleware` prefers a handler-declared `audit_actor_id` over `$request->user()`. | Found while building D2, and it decides whether D2 is possible at all. A login has no `$request->user()`: authentication ends with a token being *issued*, so the guard resolves nobody for the whole request. `actor_id` was therefore NULL on **every login row ever written** — 47 in development, the 32 successful ones included. A login history that cannot say whose account was used is not a login history, so this is entailed by D2 rather than added to it. The declaration is made **before** the password is checked, so a failed attempt on a real account is attributed: "somebody has been trying my account from an address I do not recognise" is the question a login history exists to answer, and it is unanswerable otherwise. An unknown email stays null — there is no account to name. **Trade-off:** this links an IP to an account on attempts that did not authenticate. That is the point of a security log, and the data is visible only behind `security.view` (D3) |
