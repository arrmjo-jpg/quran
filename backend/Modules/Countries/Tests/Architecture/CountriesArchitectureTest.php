...
uses()->group('countries', 'architecture');

/*
|--------------------------------------------------------------------------
| Countries Module — Architecture Tests
|--------------------------------------------------------------------------
...
|     degrades to a single `*` — so `Modules/Countries/**` reached exactly one
...
|
| Run with: php artisan test --filter CountriesArchitecture
*/

test('Countries domain layer has no framework dependencies', function (): void {
    ...
    foreach (ModuleBoundary::phpFilesUnder(base_path('Modules/Countries/Domain')) as $file) {
    ...
    fn (): bool => ModuleBoundary::phpFilesUnder(base_path('Modules/Countries/Domain')) === [],
    'Countries has no Domain layer yet. Skipped out loud rather than passing over an empty scan.'
);

test('Countries does not import concrete classes from other modules', function (): void {
    ...
    $offenders = ModuleBoundary::unbaselinedViolationsIn('Countries');
