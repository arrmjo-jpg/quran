...
uses()->group('streaming', 'architecture');

/*
|--------------------------------------------------------------------------
| Streaming Module — Architecture Tests
|--------------------------------------------------------------------------
...
|     degrades to a single `*` — so `Modules/Streaming/**` reached exactly one
...
|
| Run with: php artisan test --filter StreamingArchitecture
*/

test('Streaming domain layer has no framework dependencies', function (): void {
    ...
    foreach (ModuleBoundary::phpFilesUnder(base_path('Modules/Streaming/Domain')) as $file) {
    ...
    fn (): bool => ModuleBoundary::phpFilesUnder(base_path('Modules/Streaming/Domain')) === [],
    'Streaming has no Domain layer yet. Skipped out loud rather than passing over an empty scan.'
);

test('Streaming does not import concrete classes from other modules', function (): void {
    ...
    $offenders = ModuleBoundary::unbaselinedViolationsIn('Streaming');
