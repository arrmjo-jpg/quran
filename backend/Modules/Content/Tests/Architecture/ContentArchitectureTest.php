...
uses()->group('content', 'architecture');

/*
|--------------------------------------------------------------------------
| Content Module — Architecture Tests
|--------------------------------------------------------------------------
...
|     degrades to a single `*` — so `Modules/Content/**` reached exactly one
...
|
| Run with: php artisan test --filter ContentArchitecture
*/

test('Content domain layer has no framework dependencies', function (): void {
    ...
    foreach (ModuleBoundary::phpFilesUnder(base_path('Modules/Content/Domain')) as $file) {
    ...
    fn (): bool => ModuleBoundary::phpFilesUnder(base_path('Modules/Content/Domain')) === [],
    'Content has no Domain layer yet. Skipped out loud rather than passing over an empty scan.'
);

test('Content does not import concrete classes from other modules', function (): void {
    ...
    $offenders = ModuleBoundary::unbaselinedViolationsIn('Content');
