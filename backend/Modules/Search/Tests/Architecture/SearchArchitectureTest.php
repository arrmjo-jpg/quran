...
test('Search domain layer has no framework dependencies', function (): void {
    $files = glob(base_path('Modules/Search/Domain/**/*.php')) ?: [];
...
})->skip(fn () => ! is_dir(base_path('Modules/Search/Domain')));

test('Search does not import concrete classes from other modules', function (): void {
    $moduleFiles = glob(base_path('Modules/Search/**/*.php'), (defined('GLOB_BRACE') ? GLOB_BRACE : 0)) ?: [];
    ...
    foreach ($moduleFiles as $file) {
        ...
        $violations = array_filter($matches[1], fn (string $m): bool => $m !== 'Search');

        expect($violations)
            ->toBeEmpty("File {$file} imports concrete classes from: ".implode(', ', $violations));
    }
})->skip(fn () => empty(glob(base_path('Modules/Search/**/*.php'), (defined('GLOB_BRACE') ? GLOB_BRACE : 0))));
