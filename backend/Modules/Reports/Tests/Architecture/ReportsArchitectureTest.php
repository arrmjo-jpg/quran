...
uses()->group('reports', 'architecture');

/*
|--------------------------------------------------------------------------
| Reports Module — Architecture Tests
|--------------------------------------------------------------------------
...
|     degrades to a single `*` — so `Modules/Reports/**` reached exactly one
...
|
| Run with: php artisan test --filter ReportsArchitecture
*/

test('Reports domain layer has no framework dependencies', function (): void {
    ...
    foreach (ModuleBoundary::phpFilesUnder(base_path('Modules/Reports/Domain')) as $file) {
    ...
    fn (): bool => ModuleBoundary::phpFilesUnder(base_path('Modules/Reports/Domain')) === [],
    'Reports has no Domain layer yet. Skipped out loud rather than passing over an empty scan.'
);

test('Reports does not import concrete classes from other modules', function (): void {
    ...
    $offenders = ModuleBoundary::unbaselinedViolationsIn('Reports');
