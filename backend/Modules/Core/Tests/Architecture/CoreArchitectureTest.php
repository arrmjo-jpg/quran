...
|     degrades to a single `*` — so `Modules/Core/**` reached exactly one
...
|
| Run with: php artisan test --filter CoreArchitecture
*/

test('Core domain layer has no framework dependencies', function (): void {
    ...
    foreach (ModuleBoundary::phpFilesUnder(base_path('Modules/Core/Domain')) as $file) {
    ...
    fn (): bool => ModuleBoundary::phpFilesUnder(base_path('Modules/Core/Domain')) === [],
    'Core has no Domain layer yet. Skipped out loud rather than passing over an empty scan.'
);

test('Core does not import concrete classes from other modules', function (): void {
    ...
    $offenders = ModuleBoundary::unbaselinedViolationsIn('Core');
