...
uses()->group('applications', 'architecture');

/*
|--------------------------------------------------------------------------
| Applications Module — Architecture Tests
|--------------------------------------------------------------------------
...
|     degrades to a single `*` — so `Modules/Applications/**` reached exactly one
...
|
| Run with: php artisan test --filter ApplicationsArchitecture
*/

test('Applications domain layer has no framework dependencies', function (): void {
    ...
    foreach (ModuleBoundary::phpFilesUnder(base_path('Modules/Applications/Domain')) as $file) {
    ...
    fn (): bool => ModuleBoundary::phpFilesUnder(base_path('Modules/Applications/Domain')) === [],
    'Applications has no Domain layer yet. Skipped out loud rather than passing over an empty scan.'
);

test('Applications does not import concrete classes from other modules', function (): void {
    ...
    $offenders = ModuleBoundary::unbaselinedViolationsIn('Applications');
