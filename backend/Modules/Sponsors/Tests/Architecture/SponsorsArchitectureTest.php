...
uses()->group('sponsors', 'architecture');

/*
|--------------------------------------------------------------------------
| Sponsors Module — Architecture Tests
|--------------------------------------------------------------------------
...
|     degrades to a single `*` — so `Modules/Sponsors/**` reached exactly one
...
|
| Run with: php artisan test --filter SponsorsArchitecture
*/

test('Sponsors domain layer has no framework dependencies', function (): void {
    ...
    foreach (ModuleBoundary::phpFilesUnder(base_path('Modules/Sponsors/Domain')) as $file) {
    ...
    fn (): bool => ModuleBoundary::phpFilesUnder(base_path('Modules/Sponsors/Domain')) === [],
    'Sponsors has no Domain layer yet. Skipped out loud rather than passing over an empty scan.'
);

test('Sponsors does not import concrete classes from other modules', function (): void {
    ...
    $offenders = ModuleBoundary::unbaselinedViolationsIn('Sponsors');
