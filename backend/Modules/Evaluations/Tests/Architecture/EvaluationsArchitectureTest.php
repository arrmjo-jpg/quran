...
uses()->group('evaluations', 'architecture');

/*
|--------------------------------------------------------------------------
| Evaluations Module — Architecture Tests
|--------------------------------------------------------------------------
...
|     degrades to a single `*` — so `Modules/Evaluations/**` reached exactly one
...
|
| Run with: php artisan test --filter EvaluationsArchitecture
*/

test('Evaluations domain layer has no framework dependencies', function (): void {
    ...
    foreach (ModuleBoundary::phpFilesUnder(base_path('Modules/Evaluations/Domain')) as $file) {
    ...
    fn (): bool => ModuleBoundary::phpFilesUnder(base_path('Modules/Evaluations/Domain')) === [],
    'Evaluations has no Domain layer yet. Skipped out loud rather than passing over an empty scan.'
);

test('Evaluations does not import concrete classes from other modules', function (): void {
    ...
    $offenders = ModuleBoundary::unbaselinedViolationsIn('Evaluations');
