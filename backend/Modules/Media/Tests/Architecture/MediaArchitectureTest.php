...
uses()->group('media', 'architecture');

/*
|--------------------------------------------------------------------------
| Media Module — Architecture Tests
|--------------------------------------------------------------------------
...
|     degrades to a single `*` — so `Modules/Media/**` reached exactly one
...
|
| Run with: php artisan test --filter MediaArchitecture
*/

test('Media domain layer has no framework dependencies', function (): void {
    ...
    foreach (ModuleBoundary::phpFilesUnder(base_path('Modules/Media/Domain')) as $file) {
    ...
    fn (): bool => ModuleBoundary::phpFilesUnder(base_path('Modules/Media/Domain')) === [],
    'Media has no Domain layer yet. Skipped out loud rather than passing over an empty scan.'
);

test('Media does not import concrete classes from other modules', function (): void {
    ...
    $offenders = ModuleBoundary::unbaselinedViolationsIn('Media');
