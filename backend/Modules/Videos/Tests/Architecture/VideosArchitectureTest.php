...
uses()->group('videos', 'architecture');

/*
|--------------------------------------------------------------------------
| Videos Module — Architecture Tests
|--------------------------------------------------------------------------
...
|     degrades to a single `*` — so `Modules/Videos/**` reached exactly one
...
|
| Run with: php artisan test --filter VideosArchitecture
*/

test('Videos domain layer has no framework dependencies', function (): void {
    ...
    foreach (ModuleBoundary::phpFilesUnder(base_path('Modules/Videos/Domain')) as $file) {
    ...
    fn (): bool => ModuleBoundary::phpFilesUnder(base_path('Modules/Videos/Domain')) === [],
    'Videos has no Domain layer yet. Skipped out loud rather than passing over an empty scan.'
);

test('Videos does not import concrete classes from other modules', function (): void {
    ...
    $offenders = ModuleBoundary::unbaselinedViolationsIn('Videos');
