...
uses()->group('judges', 'architecture');

/*
|--------------------------------------------------------------------------
| Judges Module — Architecture Tests
|--------------------------------------------------------------------------
...
|     degrades to a single `*` — so `Modules/Judges/**` reached exactly one
...
|
| Run with: php artisan test --filter JudgesArchitecture
*/

test('Judges domain layer has no framework dependencies', function (): void {
    ...
    foreach (ModuleBoundary::phpFilesUnder(base_path('Modules/Judges/Domain')) as $file) {
    ...
    fn (): bool => ModuleBoundary::phpFilesUnder(base_path('Modules/Judges/Domain')) === [],
    'Judges has no Domain layer yet. Skipped out loud rather than passing over an empty scan.'
);

test('Judges does not import concrete classes from other modules', function (): void {
    ...
    $offenders = ModuleBoundary::unbaselinedViolationsIn('Judges');
