...
test('POST reopen-registration returns the open season', function (): void {
    ...
    $this->actingAs($admin)->postJson("/api/v1/admin/seasons/{$seasonId}/reopen-registration", [
        ...
        // Same projection as every other admin season response. Written on a
        // branch that predated country_ids, so this pins that the merge did
        // not leave it reporting an empty set.
        ->assertJsonStructure(['data' => ['country_ids']]);
});
