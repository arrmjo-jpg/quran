...
|     degrades to a single `*` — so `Modules/Competition/**` reached exactly one
...
test('Competition domain layer has no framework dependencies', function (): void {
    ...
    foreach (ModuleBoundary::phpFilesUnder(base_path('Modules/Competition/Domain')) as $file) {
    ...
    fn (): bool => ModuleBoundary::phpFilesUnder(base_path('Modules/Competition/Domain')) === [],
    'Competition has no Domain layer yet. Skipped out loud rather than passing over an empty scan.'
);
...
test('Competition does not import concrete classes from other modules', function (): void {
    ...
    $offenders = ModuleBoundary::unbaselinedViolationsIn('Competition');
