...
|     degrades to a single `*` — so `Modules/Organization/**` reached exactly one
...
test('Organization domain layer has no framework dependencies', function (): void {
    ...
    foreach (ModuleBoundary::phpFilesUnder(base_path('Modules/Organization/Domain')) as $file) {
    ...
    fn (): bool => ModuleBoundary::phpFilesUnder(base_path('Modules/Organization/Domain')) === [],
    'Organization has no Domain layer yet. Skipped out loud rather than passing over an empty scan.'
);
...
test('Organization does not import concrete classes from other modules', function (): void {
    ...
    $offenders = ModuleBoundary::unbaselinedViolationsIn('Organization');
