...
test('Organization does not import concrete classes from other modules', function (): void {
    $moduleFiles = glob(base_path('Modules/Organization/**/*.php'), GLOB_BRACE) ?: [];
    ...
    foreach ($moduleFiles as $file) {
        ...
        $violations = array_filter($matches[1], fn (string $m): bool => $m !== 'Organization');

        expect($violations)
            ->toBeEmpty("File {$file} imports concrete classes from: " . implode(', ', $violations));
    }
})->skip(fn () => empty(glob(base_path('Modules/Organization/**/*.php'), GLOB_BRACE)));
