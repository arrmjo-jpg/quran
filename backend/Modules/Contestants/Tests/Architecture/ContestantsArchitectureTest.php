...
uses()->group('contestants', 'architecture');

/*
|--------------------------------------------------------------------------
| Contestants Module — Architecture Tests
|--------------------------------------------------------------------------
...
|     degrades to a single `*` — so `Modules/Contestants/**` reached exactly one
...
|
| Run with: php artisan test --filter ContestantsArchitecture
*/

test('Contestants domain layer has no framework dependencies', function (): void {
    ...
    foreach (ModuleBoundary::phpFilesUnder(base_path('Modules/Contestants/Domain')) as $file) {
    ...
    fn (): bool => ModuleBoundary::phpFilesUnder(base_path('Modules/Contestants/Domain')) === [],
    'Contestants has no Domain layer yet. Skipped out loud rather than passing over an empty scan.'
);

test('Contestants does not import concrete classes from other modules', function (): void {
    ...
    $offenders = ModuleBoundary::unbaselinedViolationsIn('Contestants');
