...
uses()->group('notifications', 'architecture');

/*
|--------------------------------------------------------------------------
| Notifications Module — Architecture Tests
|--------------------------------------------------------------------------
...
|     degrades to a single `*` — so `Modules/Notifications/**` reached exactly one
...
|
| Run with: php artisan test --filter NotificationsArchitecture
*/

test('Notifications domain layer has no framework dependencies', function (): void {
    ...
    foreach (ModuleBoundary::phpFilesUnder(base_path('Modules/Notifications/Domain')) as $file) {
    ...
    fn (): bool => ModuleBoundary::phpFilesUnder(base_path('Modules/Notifications/Domain')) === [],
    'Notifications has no Domain layer yet. Skipped out loud rather than passing over an empty scan.'
);

test('Notifications does not import concrete classes from other modules', function (): void {
    ...
    $offenders = ModuleBoundary::unbaselinedViolationsIn('Notifications');
